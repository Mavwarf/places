...
cd cmd/places-app && go build -tags production -ldflags "-X main.version=$(git describe --tags --always) -X 'main.buildTime=$(date -u +%Y-%m-%d\ %H:%M\ UTC)' -H windowsgui" -o places-app.exe . && cp places-app.exe /c/dev/tools/cli/places-app.exe
```
